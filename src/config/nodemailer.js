...
const RestablecimientoContraseñaAdmin = async (userMail, token) => {
  try {
    let info = await transporter.sendMail({
      ...
      <h1>Sistema de gestión (Parking 🚗🛣️)</h1>
      <hr>
      <a href="https://backend-espacios-esfot.onrender.com/api/administrador/recuperar-clave/?token=${token}">Clic para reestablecer tu contraseña</a>
      <hr>
      <footer>Bienvenid@s!</footer>
      `,
    });
...
const RestablecimientoContraseñaGuardia = async (userMail, token) => {
  try {
    let info = await transporter.sendMail({
      ...
      <h1>Sistema de gestión (Parking 🚗🛣️)</h1>
      <hr>
      <a href="https://frontend-poliparking.vercel.app/cambiar-contrasena?token=${token}">Clic para reestablecer tu contraseña</a>
      <hr>
      <footer>Bienvenid@s!</footer>
      `,
    });
